...
\end{equation*}
Now going back to our definition of the $a$ operator, we can rewrite $Q$ as
\begin{equation*}
  Q = -i Q_\text{zpf} (a - a^\dagger)
\end{equation*}
Note that this equation holds regardless of whether or not we're dealing with a harmonic system.
In other words, it is not necessary to have $H \propto a^\dagger a$ in order to write $Q$ in this form.
Now if we restrict to any particular two levels $\ket{n}$ and $\ket{n+1}$, then the drive Hamiltonian becomes
\begin{align*}
  H_d
  &= \frac{V_d(t)}{1 + C / C_d}
    \left[ \begin{array}{cc}
    ...
    \end{array}\right] \\
  &= \frac{V_d(t)}{1 + C / C_d} \sqrt{n+1} \, Q_\text{zpf} \, \sigma_y
  \, .
\end{align*}
...
Just as in the capacitive case, we can rewrite $\Phi$ in terms of the raising and lower operators
\begin{equation*}
  \Phi = \Phi_\text{zpf} (a + a^\dagger) \, .
\end{equation*}
Again restricting to two levels $\ket{n}$ and $\ket{n+1}$, the drive Hamiltonian becomes
\begin{align*}
  H_d
  &= - \frac{M_d}{L} I_d(t)
    \left[ \begin{array}{cc}
      \bbraket{n}{\Phi}{n} & \bbraket{n}{\Phi}{n+1} \\
      \bbraket{n+1}{\Phi}{n} & \bbraket{n+1}{\Phi}{n+1} \\
    \end{array} \right] \\
  &= - \frac{M_d}{L} I_d(t) \sqrt{n+1} \, \Phi_\text{zpf} \, \sigma_x
  \, .
\end{align*}
