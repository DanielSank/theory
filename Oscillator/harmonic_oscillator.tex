...
\end{equation}
If the drive is a narrow band signal naer the resonance, we can write it as
\begin{equation}
  J(t) = E(t) \cos(\omega_0' t) \, ,
\end{equation}
in which case we have
\begin{equation}
  \tilde{J}(\omega) = \frac{1}{2}
  \left(
    \tilde{E}(\omega - \omega_0') + \tilde{E}(\omega + \omega_0')
    \right) \, .
\end{equation}
Keeping only the largest terms, we get
\begin{equation}
  \tilde{\phi}(\omega) =
  - \frac{1}{4 \omega_0'} \left( \frac{\tilde{E}(\omega - \omega_0')}{\omega - \omega_+} - \frac{\tilde{E}(\omega + \omega_0')}{\omega - \omega_-} \right) \, .
\end{equation}
Note that $\tilde{\phi}(-\omega) = \tilde{\phi}(\omega)^*$ as appropriate for $\phi(t) \in \mathbb{R}$.
Because $\phi(t)$ is real, we can write it as a Fourier transform over only positive frequencies:
\begin{align*}
  \phi(t)
  &= 2 \Re \int_0^\infty \frac{d\omega}{2\pi} \, e^{i \omega t} \tilde{\phi}(\omega) \\
  &= 2 \Re \int_0^\infty \frac{d\omega}{2\pi} \, e^{i \omega t} \frac{-1}{4 \omega_0'}
    \left(
      \frac{\tilde{E}(\omega - \omega_0')}{\omega - \omega_+}
    - \underbrace{\frac{\tilde{E}(\omega + \omega_0')}{\omega - \omega_-}}_\text{small}
    \right) \\
  (\text{define } \omega \equiv \omega_0' + \delta \omega)
  &= \Re
  \left(
    e^{i \omega_0' t}
    ...
    e^{i \delta \omega t}
    \frac{-1}{2 \omega_0'}
    \frac{\tilde{E}(\delta \omega)}{\delta \omega - i \beta}
  \right)
\end{align*}
Suppose $\tilde{E}$ is band limited to $[-\Delta \omega, \Delta \omega]$ where $\Delta \omega \ll \omega_0'$.
...
\begin{equation}
  ...
  \right)
\end{equation}
The underbraced part $\psi(t)$ is the part of the oscillator's motion that comes on top of the usual free oscillation at frequency $\omega_0'$.
By inspection, the time dynamics of $\psi(t)$ are governed by the equation
\begin{align}
  -i \dot{\psi}(t) - i \beta \psi(t) &= - \frac{E(t)}{2 \omega_0'} \\
  \dot{\psi}(t) &= - \frac{i E(t)}{2 \omega_0'} - \beta \psi(t) \, .
\end{align}
